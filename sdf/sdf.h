...
bool AllAttributesExist(const mjModel* m, int instance) {
    for (int i = 0; i < Attr::nattribute; i++) {
        if (!CheckAttr(Attr::names[i], m, instance)) {
            mju_warning("Missing attribute: %s", Attr::names[i]);
            return false;
        }
    }
    return true;
